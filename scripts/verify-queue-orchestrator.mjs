...
const processedTimelineItem = timeline.items.find(
  (item) => item.title === "Queue Item Processed"
);
...
assert(
  processedDecision,
  "queue_execution_processed decision should be created"
);
assert(
...
assert(
  processedDecision.decision.outcome.result === "processed",
  "processed decision should store processed result"
);
assert(
...
assert(
  processedTimelineItem?.message ===
    "Operations Agent processed the resumed execution queue item.",
